...
void loop() {
    ...
    if ((millis() - timer) > PUBLISH_FREQUENCY) {
        ...
        if (nomePessoa.length() > 0) {
            ...
            char context[100];
            snprintf(context, sizeof(context), "\"nome\":\"%s\",\"localizacao\":\"%s\",\"cargo\":\"%s\"", nomePessoa.c_str(), localizacao, cargo.c_str());
...
void leituraDados() {
 ...
 // Make sure to clear the buffer to avoid garbage from previous reads
  ...
  // Authenticate the next block for operation
  MFRC522::StatusCode status2 = mfrc522.PCD_Authenticate(
      MFRC522::PICC_CMD_MF_AUTH_KEY_A, blocoCargo, &key, &(mfrc522.uid));

  // Check if authentication was successful for the second block
  if (status2 != MFRC522::STATUS_OK) {
      Serial.print(F("Authentication failed for blocoCargo: "));
      Serial.println(mfrc522.GetStatusCodeName(status2));
      return;
  }

  // Read data from the authenticated blocoCargo
  status2 = mfrc522.MIFARE_Read(blocoCargo, bufferCargo, &tamanho);

  // Check if reading was successful for blocoCargo
  if (status2 != MFRC522::STATUS_OK) {
      Serial.print(F("Reading failed for blocoCargo: "));
      Serial.println(mfrc522.GetStatusCodeName(status2));
      return;
      ...
      for (int i = 0; i < 16; i++) {// Assume the name is in the first 16 bytes of the buffer
              // Add only printable ASCII characters
        if (bufferCargo[i] >= 32 && bufferCargo[i] <= 126) {
          cargo += (char)bufferCargo[i];
        } else if (bufferCargo[i] == '\0') {
          break;// If a null byte is encountered, stop adding
        }
      }
      
      cargo.trim();// Remove whitespace from the beginning and end
          // Print the person's name or indicate if it's empty
      if (cargo.length() > 0) {
